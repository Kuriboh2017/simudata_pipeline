...
def _run_fisheye(input_dir, output_dir, subfolder, args, panorama_to_fisheye, fish_to_3pinholes):
    ...
    run_fisheye_exe = distutils.spawn.find_executable(
        "run_fisheye_isp_combo.py")
    assert run_fisheye_exe, 'Error: python executable `run_fisheye_isp_combo.py` is not available!'
    cmds = [
        run_fisheye_exe,
...
def _main(args):
    ...
    _run_fisheye(input_dir, output_dir, left_rgba_folder, args, left_p2f_path, f2p_path)
    _run_fisheye(input_dir, output_dir, right_rgba_folder, args, left_p2f_path, f2p_path)
