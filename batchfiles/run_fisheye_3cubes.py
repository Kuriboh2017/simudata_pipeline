...
def _run_fisheye(input_dir, output_dir, subfolder, args, panorama_to_fisheye, fish_to_3pinholes, seed, is_right_cam):
    ...
    if is_right_cam:
        cmds.extend(('--use-right-intrinsics', '--noisy', '--regenerate-noise'))
    else:
...
def _main(args):
    ...
    front_rgba_folder = Path('.') / 'cube_front' / 'CubeScene'
    rear_rgba_folder = Path('.') / 'cube_rear' / 'CubeScene'
    below_rgba_folder = Path('.') / 'cube_below' / 'CubeScene'
    ...
    _run_fisheye(input_dir, output_dir, front_rgba_folder, args, left_p2f_path, f2p_path, seed,
                 is_right_cam=False)
    _run_fisheye(input_dir, output_dir, rear_rgba_folder, args, left_p2f_path, f2p_path, seed,
                 is_right_cam=False)
    _run_fisheye(input_dir, output_dir, below_rgba_folder, args, left_p2f_path, f2p_path, seed,
                 is_right_cam=False)
