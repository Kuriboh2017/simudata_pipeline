...
def _panorama_to_fisheye(input_file, args, scale_factor=None):
    ...
    if args.random_roll_max == 0.0:
        ...
        seed = get_seed_from_filename(input_file)
        random.seed(seed)
    ...
    elif args.random_roll_max == 0.0 and args.panorama_to_fisheye:
        cmds.append(
...
def _add_blur(fisheye_file, step, args):
    ...
    cmds = [add_blur_exe, f'--input-image-path={fisheye_file}']
    output_filepath = intermediate_dir / \
        _append_filename(fisheye_file.name, f'{step}_blur')
...
def _add_vignette(fisheye_file, step, args):
    ...
    output_filepath = intermediate_dir / \
        _append_filename(fisheye_file.name, f'{step}_vignette')
    cmds.append(f'--output-image-path={str(output_filepath)}')
    seed = get_seed_from_filename(fisheye_file)
    cmds.append('--random')
    cmds.append(f'--random-seed={seed}')
    _logger.info(f'Executing command:\n{" ".join(cmds)}')
    run(cmds)
    return output_filepath


def _add_demosaicing(fisheye_file, step, args):
    ...
    cmds = [action_exe, f'--input-image-path={fisheye_file}']
    output_filepath = intermediate_dir / \
        _append_filename(fisheye_file.name, f'{step}_demosaicing')
    cmds.append(f'--output-image-path={str(output_filepath)}')
    _logger.info(f'Executing command:\n{" ".join(cmds)}')
    run(cmds)
    return output_filepath
...
def _add_fixed_isp(fisheye_file, step, args):
    ...
    cmds = [action_exe, f'--input-image-path={fisheye_file}']
    output_filepath = intermediate_dir / \
        _append_filename(fisheye_file.name, f'{step}_fixed_isp')
    cmds.extend(
...
def _add_random_isp(fisheye_file, step, args):
    action_exe = distutils.spawn.find_executable('add_isp.py')
    assert action_exe, 'Error: python executable `add_isp.py` is not available!'
    intermediate_dir = Path(args.output_dir) / kIntermediateDir / 'random_isp'
    cmds = [action_exe, f'--input-image-path={fisheye_file}']
    output_filepath = intermediate_dir / \
        _append_filename(fisheye_file.name, f'{step}_random_isp')
    cmds.extend(
...
def _add_dark_isp(fisheye_file, step, args):
    action_exe = distutils.spawn.find_executable('add_isp.py')
    assert action_exe, 'Error: python executable `add_isp.py` is not available!'
    intermediate_dir = Path(args.output_dir) / kIntermediateDir / 'dark_isp'
    cmds = [action_exe, f'--input-image-path={fisheye_file}']
    output_filepath = intermediate_dir / \
        _append_filename(fisheye_file.name, f'{step}_dark_isp')
...
def _add_sharpen(fisheye_file, step, args):
    action_exe = distutils.spawn.find_executable('add_sharpen.py')
    assert action_exe, 'Error: python executable `add_sharpen.py` is not available!'
    intermediate_dir = Path(args.output_dir) / kIntermediateDir / 'sharpen'
    cmds = [action_exe, f'--input-image-path={fisheye_file}']
    output_filepath = intermediate_dir / \
        _append_filename(fisheye_file.name, f'{step}_sharpen')
    cmds.append(f'--output-image-path={str(output_filepath)}')
    seed = get_seed_from_filename(fisheye_file)
    cmds.append('--random')
    cmds.append(f'--random-seed={seed}')
...
def _add_clache(fisheye_file, step, args):
    action_exe = distutils.spawn.find_executable('add_clache.py')
    assert action_exe, 'Error: python executable `add_clache.py` is not available!'
    intermediate_dir = Path(args.output_dir) / kIntermediateDir / 'clache'
    cmds = [action_exe, f'--input-image-path={fisheye_file}']
    output_filepath = intermediate_dir / \
        _append_filename(fisheye_file.name, f'{step}_clache')
    cmds.append(f'--output-image-path={str(output_filepath)}')
    _logger.info(f'Executing command:\n{" ".join(cmds)}')
    run(cmds)
    return output_filepath
...
def _fisheye_to_3pinholes(input_file, args, output_filename=None):
    '''
    Input is a single round fisheye image filepath.
    Output is a single remapped 3pinholes image filepath.
    '''
    assert (isinstance(input_file, Path))
    ...
    if output_filename is None:
        ...
        output_filepath = output_dir / Path(output_filename)
    cmds.append(f'--output-image-path={str(output_filepath)}')
    if args.rectify_config is not None and args.rectify_config != 'None':
        cmds.append(f'--rectify-config={args.rectify_config}')
...
def _fisheye_to_erp(input_file, args, output_filename=None):
    '''
    Input is a single round fisheye image filepath.
    Output is a single remapped erp image filepath.
    '''
    assert (isinstance(input_file, Path))
    output_dir = Path(args.output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)

    fisheye_to_erp_exe = distutils.spawn.find_executable(
        "fisheye_to_erp.py")
    assert fisheye_to_erp_exe, 'Error: executable `fisheye_to_erp.py` is not available!'
    cmds = [fisheye_to_erp_exe, f'--input-image-path={input_file}']
    if output_filename is None:
        output_filepath = output_dir / input_file.name
    else:
        output_filepath = output_dir / Path(output_filename)
    output_filepath_str = str(output_filepath).replace('_3to1', '_erp')
    cmds.append(f'--output-image-path={output_filepath_str}')
    _logger.info(f'Executing command:\n{" ".join(cmds)}')
    run(cmds)
    return output_filepath


def _extract_filename(original_name):
    # Remove the step and effect names.
    stem = Path(original_name).stem
    suffix = Path(original_name).suffix
    parts = stem.split("_")
    if len(parts) < 2 or (parts[1] not in ['down', 'up']):
        assert False, f"Unexpected filename format: {original_name}"
    return f"{parts[0]}_{parts[1]}{suffix}"


def _run_with_effects(input_file, args, effect_functions):
    fisheye_file = input_file
    for step, effect_func in enumerate(effect_functions):
        fisheye_file = effect_func(fisheye_file, step, args)
    img_3pinholes = _fisheye_to_3pinholes(
        fisheye_file, args, _extract_filename(input_file.name))
    img_erp = _fisheye_to_erp(
        fisheye_file, args, _extract_filename(input_file.name))
    if img_3pinholes.is_file():
        _logger.info(
            f'Generated a 3pinholes image file : {img_3pinholes}')
    if img_erp.is_file():
        _logger.info(
            f'Generated a erp image file : {img_erp}')


def _no_effect(input_file, step, args):
    return input_file
...
def _process_fisheyes(fisheye_files, args):
    for fisheye_file in fisheye_files:
        ...
        if add_effect := rng.choice([True, False], p=[0.95, 0.05]):
            _run_with_effects(fisheye_file, args, [
                _add_vignette, _add_blur, _add_demosaicing,
                _add_random_isp, _add_sharpen])
        else:
            _run_with_effects(fisheye_file, args, [_no_effect])
