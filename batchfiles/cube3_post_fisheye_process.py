...
def _copy_file(file, src_dir, dest_dir, args):
    ...
    elif relative_path.parts[0] in (f'CubeDisparity_{str(args.up_baseline)}_erp'):
        relative_path = Path(f'Disparity_{str(args.up_baseline)}_erp').joinpath(
            *relative_path.parts[1:])
    elif relative_path.parts[0] in (f'CubeDisparity_{str(args.down_baseline)}_erp'):
        relative_path = Path(f'Disparity_{str(args.down_baseline)}_erp').joinpath(
            *relative_path.parts[1:])
...
def _copy_files_if_with_suffix(src_dir, dest_dir, suffix, args):
    ...
    with Pool(cpu_count()) as p:
        p.map(partial(_copy_file, src_dir=src_dir,
              dest_dir=dest_dir, args=args), filtered_files)
...
def _convert_depth_to_disparity(dir, baseline, args):
    ...
    target_dir = dir.parent / 'Disparity_3to1'
    if target_dir.exists():
        shutil.rmtree(target_dir)
    dir.rename(target_dir)


def _handle_erp_disparity_folders(down_dir, up_dir, args):
    down_disparity_erp = down_dir / f'Disparity_{str(args.down_baseline)}_erp'
    up_disparity_erp = up_dir / f'Disparity_{str(args.up_baseline)}_erp'
    ...
    shutil.rmtree(down_dir / f'Disparity_{str(args.up_baseline)}_erp')
    shutil.rmtree(up_dir / f'Disparity_{str(args.down_baseline)}_erp')
...
def _process_dir(output_dir):
    ...
    _copy_files_if_with_suffix(cube_front_dir, group0_0_dir, '_down', args)
    _copy_files_if_with_suffix(cube_front_dir, group1_1_dir, '_up', args)
    _copy_files_if_with_suffix(cube_rear_dir, group1_0_dir, '_up', args)
    _copy_files_if_with_suffix(cube_below_dir, group0_1_dir, '_down', args)
    ...
    _convert_depth_to_disparity(group0_0_dir / f'{DEPTH}', args.down_baseline , args)
    # Up camera baseline: 0.09
    _convert_depth_to_disparity(group1_0_dir / f'{DEPTH}', args.up_baseline, args)
    # Handle ERP disparity folders
    _handle_erp_disparity_folders(group0_0_dir, group1_0_dir, args)
