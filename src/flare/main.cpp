...
int main(int argc, char *argv[]) {
  ...
  const int lens_flares_count = FLAGS_lens_flares_count;
  const float lens_flares_seed = static_cast<float>(FLAGS_lens_flares_seed);
  ...
  std::filesystem::path vertex_shader = current_folder / "flare.vs";
  std::filesystem::path fragment_shader = current_folder / "flare.fs";
  ...
  glUniform1i(glGetUniformLocation(ourShader.id(), "lens_flares_count"),
              lens_flares_count);
  glUniform1f(glGetUniformLocation(ourShader.id(), "lens_flares_seed"),
              lens_flares_seed);
